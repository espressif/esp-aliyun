...
static void esp_zeroize( void *v, size_t n ) {
...
void esp_aes_free( esp_aes_context *ctx )
{
    ...
    esp_zeroize( ctx, sizeof( esp_aes_context ) );
}
                    ...
                    unsigned int keybits )
{
    ...
    switch( keybits )
    {
        ...
        default : return( ESP_ERR_AES_INVALID_KEY_LENGTH );
    }
                    ...
                    unsigned int keybits )
{
    ...
    /* Also checks keybits */
    if( ( ret = esp_aes_setkey_enc( &cty, key, keybits ) ) != 0 )
        goto exit;
                    ...
                    unsigned char output[16] )
{
    if( mode == ESP_AES_ENCRYPT )
        return( esp_internal_aes_encrypt( ctx, input, output ) );
    else
        return( esp_internal_aes_decrypt( ctx, input, output ) );
}
                    ...
                    unsigned char *output )
{
    ...
    if( length % 16 )
        return( ESP_ERR_AES_INVALID_INPUT_LENGTH );
                       ...
                       unsigned char *output )
{
    ...
    if( mode == ESP_AES_DECRYPT )
    {
        ...
        {
            if( n == 0 )
            	esp_aes_crypt_ecb( ctx, ESP_AES_ENCRYPT, iv, iv );
